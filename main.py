...
# Sentar: usa queda do quadril normalizada + dobra do joelho. Implementado como FSM simples:
...
def main():
    ...
    if not cap.isOpened():
        print("Erro ao abrir vídeo/webcam."); return
    pose = MediaPipePose()
    t0 = time.time()
    count_arm = count_squat = count_sit = 0

    # Logs
    events = open("out/events.csv","w", newline="", encoding="utf-8"); ew = csv.writer(events); ew.writerow(["t","gesture"])
    kpf = open("out/keypoints.csv","w", newline="", encoding="utf-8"); kw = csv.writer(kpf)
    kw.writerow(["t","ls.x","ls.y","rs.x","rs.y","lh.x","lh.y","rh.x","rh.y","le.x","le.y","re.x","re.y","lw.x","lw.y","rw.x","rw.y","lk.x","lk.y","rk.x","rk.y","la.x","la.y","ra.x","ra.y",
